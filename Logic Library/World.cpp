...
void World::move(string direction, double multiplier) {
    ...
    shared_ptr<StopWatch> stopWatch = StopWatch::getInstance();
    if (direction == "Up") {
        pacman->setPosition(make_pair(pacman->getPosition().first,
                                      pacman->getPosition().second - multiplier * stopWatch->getDeltaTime().count()));
        pacman->moving = "Up";
    } else if (direction == "Left") {
        pacman->setPosition(make_pair(pacman->getPosition().first - multiplier * stopWatch->getDeltaTime().count(),
                                      pacman->getPosition().second));
        pacman->moving = "Left";
...
void World::ghostMove(int ghost, const string &direction, double multiplier) {
    ...
    // move the ghost
    if (direction == "Up") {
        ghosts[ghost]->setPosition(make_pair(ghosts[ghost]->getPosition().first, ghosts[ghost]->getPosition().second -
                                                                                 multiplier *
                                                                                 stopWatch->getDeltaTime().count() *
                                                                                 dsi));
        ghosts[ghost]->moving = "Up";
    } else if (direction == "Left") {
        ghosts[ghost]->setPosition(
                make_pair(ghosts[ghost]->getPosition().first - multiplier * stopWatch->getDeltaTime().count() * dsi,
                          ghosts[ghost]->getPosition().second));
