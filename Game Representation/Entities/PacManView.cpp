...
PacManView::PacManView(std::shared_ptr<PacManModel> pacManModel) : pacManModel(pacManModel) {
    ...
    for (auto i: pacmanRect) {
        auto pacman = make_shared<Sprite::Sprite>();
        ...
        pair<double, double> position = camera.modelEntityPosition(pacManModel->getPosition().first,
